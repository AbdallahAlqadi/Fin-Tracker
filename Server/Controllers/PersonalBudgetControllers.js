...
// exports.getUserBudget = async (req, res) => {
//     try {
//         const budget = await Budget.findOne({ userId: req.user }).populate('products.itemId');
//         res.status(200).json(budget);
//     } catch (error) {
//         res.status(500).json({ error: error.message });
//     }
...
// // Add budget item
// exports.addBudget = async (req, res) => {
//     const { itemId, kind, valueitem, date } = req.body;
//     const userId = req.user;
...
//         if (existingBudget) {
//             return res.status(400).json({ error: 'لا يمكنك إضافة نفس العنصر في نفس التاريخ.' });
//         }
...
//             res.status(200).json(budget);
//         } else {
//             res.status(404).json({ error: 'Item not found in budget for the specified date' });
//         }
//     } catch (error) {
//         res.status(500).json({ error: error.message });
//     }
...
// // Delete budget item
// exports.deleteBudget = async (req, res) => {
//     const { itemId, kind, date } = req.body;
//     const userId = req.user;
...
//             budget.products.splice(budgetIndex, 1);
//             await budget.save();
//             res.status(200).json({ message: 'Item deleted successfully', budget });
//         } else {
//             res.status(404).json({ error: 'Item not found in budget for the specified date' });
//         }
//     } catch (error) {
